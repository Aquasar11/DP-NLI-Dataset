...
class DatabaseSchemaGenerator:
  ...
  def _load_schema_into_cache(cls, db_id: str, db_path: str) -> None:
    ...
    for table_name in db_schema.tables.keys():
      ...
      for col in columns:
        ...
        unique_values = execute_sql(db_path, query, "all", 680)
        ...
        try:
          ...
          """
          value_statics = execute_sql(db_path, value_statics_query, "all", 680)
          schema_with_type[table_name][col[1]].update({
              "value_statics": (
  ...
  def generate_schema_string(
    ...
    for table_name, ddl_command in ddl_commands.items():
      ...
      for column_def in definitions:
        ...
        if any(
          ...
          if use_fk_constraints and "foreign key" in column_def.lower():
    ...
    if return_list:
      return list(ddl_commands.values())
