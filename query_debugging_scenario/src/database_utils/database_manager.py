...
from ..llms.llm_engine import get_embedding
...
def get_database_schema_string(
  ...
  schema_string = schema_generator.generate_schema_string(
      include_value_description=include_value_description, use_fk_constraints=use_fk_constraints,
...
def get_database_schema(
  ...
  database_info = get_database_schema_string(
      ...
      use_fk_constraints=use_fk_constraints,
      return_list=return_list,
  )
  return database_info


def get_db_schema_db_id(db_id: str, bird_database_path: str, queries: List[str] = None, tentative_schema = None, use_fk_constraints = True) -> str:
  """Generates a schema string for the database.
...
def get_db_schema_db_id_with_schema_similarity(db_id: str, question: str, bird_database_path: str, queries: List[str] = None, tentative_schema = None, use_fk_constraints = True, top_k: int = 5) -> str:
  """Generates a schema string for the database.
  ...
  if top_k >= len(db_schema):
    return "\n\n".join(db_schema)

  question_embedding = get_embedding(question)
  schema_embeddings = [get_embedding(schema) for schema in db_schema]

  ## find cosine similarity and get top k
  similarity_scores = [cosine_similarity(question_embedding, schema_emb) for schema_emb in schema_embeddings]
  top_k_indices = sorted(range(len(similarity_scores)), key=lambda i: similarity_scores[i], reverse=True)[:top_k]
  db_schema = [db_schema[i] for i in top_k_indices] 
